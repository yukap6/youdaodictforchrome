...
function onScrTrans(event){
    ...
    //save no use action
    moveTimeout? clearTimeout(moveTimeout): void(0);
...
//calculator space number of string
...
function createPopUp(word,senctence, x, y, screenX, screenY) {
  ...
  //页面缩放后，获取到的这个selection_end_position还是页面缩放100%的值[也就是未缩放时候的值]
  ...
  var isZoom = window.outerWidth/window.innerWidth != 1? true: false;
